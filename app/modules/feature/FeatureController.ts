...
export default class FeatureController {
  ...
  public getFeatureRequests = async ({
    ...
    const searchFilter = search_term
      ? {
          $or: [
            ...
            { description: { $regex: search_term, $options: "i" } }, // Case-insensitive description search
          ],
        }
      : {};

    // Find feature requests that match the search filter
    const feature_requests = await this.Feature.find(searchFilter)
      .skip(skipCount)
      .limit(limit);
    ...
    return { feature_requests, totalPages };
    // } catch (error) {
  ...
  public getUserRequests = async ({ user }: { user: string }) => {
    try {
      const feature_requests = await this.Feature.find({ user });
  ...
  /**
   * Delete an item from the feature_requests
   * @param id String
   * @returns null
   */
